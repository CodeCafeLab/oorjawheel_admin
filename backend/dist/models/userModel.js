...
export async function getUserProfileById(id) {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.execute("SELECT id, email, fullName FROM users WHERE id = ? LIMIT 1", [id]);
        return rows[0] || null;
    }
    finally {
        conn.release();
...
export async function changeUserPassword(id, { oldPassword, newPassword }) {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.execute("SELECT password_hash FROM users WHERE id = ?", [id]);
