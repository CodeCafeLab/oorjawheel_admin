...
export async function create(data) {
    ...
    try {
        await query("INSERT INTO command_logs (id, device_id, user_id, command, sent_at, result, type, status, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", sanitizedParams);
...
export async function update(id, data) {
    ...
    await query("UPDATE command_logs SET device_id=?, user_id=?, command=?, sent_at=?, result=?, type=?, status=?, details=? WHERE id=?", sanitizedParams);
    return { id, ...data };
}
