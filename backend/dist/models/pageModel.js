...
export async function createPage({ title, order = 0, is_published = 1, command = null, description = null }) {
    const conn = await pool.getConnection();
    try {
        const [result] = await conn.execute('INSERT INTO pages (title, `order`, is_published, command, description) VALUES (?, ?, ?, ?, ?)', [title, order, is_published, command, description]);
...
export async function updatePage(id, { title, order = 0, is_published = 1, command = null, description = null }) {
    const conn = await pool.getConnection();
    try {
        await conn.execute('UPDATE pages SET title = ?, `order` = ?, is_published = ?, command = ?, description = ? WHERE id = ?', [title, order, is_published, command, description, id]);
        return true;
    }
    finally {
        conn.release();
