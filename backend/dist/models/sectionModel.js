...
export async function createSection({ page_id, title, order = 0 }) {
    const conn = await pool.getConnection();
    try {
        const [result] = await conn.execute('INSERT INTO sections (page_id, title, `order`) VALUES (?, ?, ?)', [page_id, title, order]);
...
export async function getSectionById(id) {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.execute('SELECT * FROM sections WHERE id = ?', [id]);
        return rows[0] || null;
    }
    finally {
        conn.release();
...
export async function updateSection(id, { page_id, title, order = 0 }) {
    const conn = await pool.getConnection();
    try {
        await conn.execute('UPDATE sections SET page_id = ?, title = ?, `order` = ? WHERE id = ?', [page_id, title, order, id]);
        return true;
    }
    finally {
        conn.release();
