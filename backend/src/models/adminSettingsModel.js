...
export async function upsertAdminGeneralSettings(userId, payload) {
  ...
  try {
    ...
    await conn.execute(
      `INSERT INTO admin_general_settings (admin_id, ${fields.join(',')}) VALUES (?, ${fields.map(()=>'?').join(',')})
...
export async function upsertAdminNotificationSettings(userId, payload) {
  ...
  try {
    ...
    await conn.execute(
      `INSERT INTO admin_notification_settings (admin_id, ${fields.join(',')}) VALUES (?, ${fields.map(()=>'?').join(',')})
       ON DUPLICATE KEY UPDATE ${fields.map(f=>`${f}=VALUES(${f})`).join(', ')}`
      , [userId, ...values]
    );
    return true;
