...
export async function getContentTypeById(id) {
  ...
  try {
    const [rows] = await conn.execute(
      'SELECT * FROM content_types WHERE id = ? AND is_active = TRUE',
      [id]
    );
    return rows[0] || null;
  } finally {
...
export async function getContentTypeWithFields(id) {
  const conn = await pool.getConnection();
  try {
    const [contentType] = await conn.execute(
      'SELECT * FROM content_types WHERE id = ? AND is_active = TRUE',
      [id]
    );
    
    if (!contentType[0]) return null;
...
export async function getContentItemsByCategory({ category_id, status = 'published', search }) {
  ...
  try {
    ...
    const [contentItems] = await conn.execute(
      `SELECT 
         ci.id,
         ci.title,
...
export async function createMediaFile({ filename, original_name, file_path, file_size, mime_type, alt_text, caption, uploaded_by }) {
  ...
  try {
    const [result] = await conn.execute(
      'INSERT INTO media_files (filename, original_name, file_path, file_size, mime_type, alt_text, caption, uploaded_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [filename, original_name, file_path, file_size, mime_type, alt_text, caption, uploaded_by]
    );
    return { id: result.insertId };
  } finally {
    conn.release();
...
export async function getFieldTypes() {
  ...
  try {
    const [rows] = await conn.execute(
      'SELECT * FROM field_types WHERE is_active = TRUE ORDER BY name'
    );
    return rows;
  } finally {
    conn.release();
...
export async function updateContentType(id, { name, slug, description, icon, is_active }) {
  ...
  try {
    await conn.execute(
      'UPDATE content_types SET name = ?, slug = ?, description = ?, icon = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
...
export async function updateCategory(id, { name, slug, description, parent_id, color, icon, is_active }) {
  ...
  try {
    await conn.execute(
      'UPDATE categories SET name = ?, slug = ?, description = ?, parent_id = ?, color = ?, icon = ?, is_active = ? WHERE id = ?',
      [name, slug, description, parent_id, color, icon, is_active, id]
    );
    return true;
  } finally {
    conn.release();
...
export async function getStaticContent(pageType) {
  ...
  try {
    const [rows] = await conn.execute(
      'SELECT * FROM static_content WHERE page_type = ? AND is_active = TRUE',
      [pageType]
    );
    return rows[0] || null;
  } finally {
...
export async function getAllStaticContent() {
  ...
  try {
    const [rows] = await conn.execute(
      'SELECT * FROM static_content WHERE is_active = TRUE ORDER BY page_type'
    );
    return rows;
  } finally {
    conn.release();
...
export async function createStaticContent({ page_type, title, content, created_by }) {
  ...
  try {
    const [result] = await conn.execute(
      'INSERT INTO static_content (page_type, title, content, created_by) VALUES (?, ?, ?, ?)',
      [page_type, title, content, created_by]
    );
    return { id: result.insertId };
  } finally {
    conn.release();
...
export async function updateStaticContent(pageType, { title, content, updated_by }) {
  ...
  try {
    const [result] = await conn.execute(
      'UPDATE static_content SET title = ?, content = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE page_type = ?',
      [title, content, updated_by, pageType]
    );
...
export async function deleteStaticContent(pageType) {
  ...
  try {
    const [result] = await conn.execute(
      'UPDATE static_content SET is_active = FALSE WHERE page_type = ?',
      [pageType]
    );
    return result.affectedRows > 0;
  } finally {
