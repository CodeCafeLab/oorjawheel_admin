...
export async function getAdminGeneral(req, res, next) {
  try { const id = await resolveUserId(req); const data = await getAdminGeneralSettings(id); res.json({ data }); } catch (err) { next(err); }
}

export async function putAdminGeneral(req, res, next) {
  try { const id = await resolveUserId(req); await upsertAdminGeneralSettings(id, req.body || {}); res.json({ message: 'General settings saved' }); } catch (err) { next(err); }
}

// Notifications per-admin
export async function getAdminNotifications(req, res, next) {
  try { const id = await resolveUserId(req); const data = await getAdminNotificationSettings(id); res.json({ data }); } catch (err) { next(err); }
}

export async function putAdminNotifications(req, res, next) {
  try { const id = await resolveUserId(req); await upsertAdminNotificationSettings(id, req.body || {}); res.json({ message: 'Notification settings saved' }); } catch (err) { next(err); }
}
