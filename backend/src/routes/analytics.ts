...
router.get('/', async (_req, res, next) => {
  try {
    ...
    try { const [row] = await query('SELECT COUNT(*) as count FROM devices WHERE status = "active"'); activeDevices = (row as any)?.count ?? 0; } catch {}
    try { const [row] = await query('SELECT COUNT(*) as count FROM users'); totalUsers = (row as any)?.count ?? 0; } catch {}
    try { const [row] = await query('SELECT COUNT(*) as count FROM device_events WHERE timestamp >= (NOW() - INTERVAL 1 DAY)'); commands24h = (row as any)?.count ?? 0; } catch {}
    try { const [row] = await query('SELECT COUNT(*) as count FROM devices WHERE warranty_start IS NOT NULL AND YEAR(warranty_start)=YEAR(CURDATE()) AND MONTH(warranty_start)=MONTH(CURDATE())'); warrantyThisMonth = (row as any)?.count ?? 0; } catch {}
    ...
    try { deviceStatus = await query(`SELECT status as status, COUNT(*) as count FROM devices GROUP BY status`); } catch {}
