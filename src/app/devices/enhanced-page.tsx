...
export default function EnhancedDevicesPage() {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const [devicesData, mastersData, devicesCount, mastersResponse] =
        await Promise.all([
  ...
  const handleDeleteSelectedDevices = (ids: string[]) => {
    Promise.all(ids.map((id) => deleteDevice(id))).then(() => {
      toast({
        ...
        description: `${ids.length} devices have been deleted.`,
      });
      refreshData();
    });
  };
  ...
  const handleDeleteSelectedMasters = (ids: string[]) => {
    Promise.all(ids.map((id) => deleteDeviceMaster(id))).then(() => {
      toast({
        title: "Device Types Deleted",
        description: `${ids.length} device types have been deleted.`,
      });
      refreshData();
    });
  };
  ...
  const handleDeviceSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const deviceData = {
      ...
      passcode: '123456', // Default passcode, you might want to generate this or make it configurable
      status: (formData.get("status") as "never_used" | "active" | "disabled") || 
    ...
    try {
      let result;
      if (selectedDevice) {
        result = await updateDevice(String(selectedDevice.id), deviceData);
    } else {
        result = await addDevice(deviceData);
      }

      if (result.success) {
        toast({
          title: selectedDevice ? "Device Updated" : "Device Created",
          description: result.message,
        });
        refreshData();
  ...
  const handleMasterSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      toast({
        title: selectedMaster ? "Device Type Updated" : "Device Type Created",
        description: result.message,
      });
      refreshData();
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-6 py-4">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              name="device-type"
                              defaultValue={selectedDevice?.deviceType ?? ""}
                            >
