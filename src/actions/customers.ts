...
export async function fetchCustomers(): Promise<Customer[]> {
  try {
    const { data } = await apiClient.get('/users', { params: { page: 1, limit: 1000 } });
...
export async function updateCustomer(id: string, values: z.infer<typeof CustomerFormSchema>) {
  try {
    await apiClient.put(`/users/${id}`, { fullName: values.name, email: values.email, status: values.status });
    revalidatePath('/customers');
